...
{
	float scalar = (vector2D_A->x) * (vector2D_B->x) + (vector2D_A->y * vector2D_B->y);
	return scalar;
}
...
{
 	float scalar = (vector3D_A->x * vector3D_B->x) + (vector3D_A->y * vector3D_B->y) + (vector3D_A->z * vector3D_B->z);
	return scalar;
}
...
float GraphicsMath_Point_Angle(const GraphicsMath_Point* reference, const GraphicsMath_Point* point)
{
	...
	float angle =  GraphicsMath_RadToDegree((atan(x/y)));
	return angle;
}
...
GraphicsMath_Linear GraphicsMath_Linear_Init(const float a, const float b)
{
	GraphicsMath_Linear linear = {linear.a = a, linear.b=b};
	return linear;
}
